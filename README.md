...
**NovelMind AI** 是一款专为**超长篇小说**（千万字级别）设计的深度分析工具。它利用最新的大语言模型（Google Gemini / OpenAI），在浏览器端对小说进行智能分章、剧情摘要、人物性格提取以及关系图谱构建。
...
### 2. 🧠 AI 深度分析
- **一键全书分析 (New)**：支持自动从第一章开始，按顺序分析全本小说。系统会自动处理 API 调用队列，您只需点击一次即可挂机等待。
- **上下文连贯性 (New)**：**强制顺序分析**。为了保证 AI 能够记住前文的伏笔和人物关系，分析必须按章节顺序进行。第 N 章的分析依赖于 第 N-1 章的摘要。
- **自定义提示词**：在设置中修改 AI 的 System Prompt，定制您专属的分析风格（如侧重“伏笔挖掘”或“心理分析”）。
- **剧情摘要**：为每一章生成精炼的剧情概括。
...
3. **自定义提示词**：如果您对默认的分析结果不满意，可以在设置中修改 Prompt。
4. **分段大小**：默认为 5万字/段。对于超长篇小说，建议适当调大此数值以减少 API 调用次数。

### 第三步：开始分析
- **方式一：一键全书 (推荐)**
  点击左侧边栏的 **“一键全书分析”** 按钮。系统将自动从第一章开始逐章分析，直到全书完成或手动停止。
- **方式二：手动分析**
...
**Q: 为什么不能直接分析第 100 章？**
A: 因为 AI 需要知道前 99 章发生了什么（人物关系、伏笔），才能准确分析第 100 章。请使用“一键全书分析”自动跑完前面的章节。
