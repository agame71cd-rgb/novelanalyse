...
**NovelMind AI** 是一款专为**超长篇小说**（1000万字级别）设计的深度分析工具。它利用最新的大语言模型（Google Gemini / OpenAI），在浏览器端对小说进行智能分章、剧情摘要、人物性格提取以及关系图谱构建。
...
### 1. 📚 长篇文本处理
- **超大规模支持**：经优化可支持 **1000万字** 以上的超长篇小说。
- **智能分章**：自动识别小说章节（如“第1章”、“序章”），或根据字数智能切分。
- **超大文件支持**：轻松处理 10MB+ 的 `.txt` 纯文本文件，支持 GBK/UTF-8 编码自动识别。
- **阅读进度记忆**：自动记录并恢复上次阅读的位置。

### 2. 🧠 AI 深度分析
- **一键全书分析 (后台运行)**：支持自动从第一章开始，按顺序分析全本小说。
  - **不干扰阅读**：分析在后台静默进行，您可以同时阅读其他章节或查看之前的分析结果。
  - **自动挂机**：系统会自动处理 API 调用队列，您只需点击一次即可挂机等待。
- **上下文连贯性 (强制)**：**顺序分析原则**。为了保证 AI 能够记住前文的伏笔和人物关系，分析必须按章节顺序进行。手动分析时，若上一章未完成，当前章将被锁定。
- **自定义提示词 (New)**：在设置中修改 AI 的 System Prompt，定制您专属的分析风格（如侧重“伏笔挖掘”或“心理分析”）。
- **剧情摘要**：为每一章生成精炼的剧情概括。
...
3. **自定义提示词**：如果您对默认的分析结果不满意，可以在设置中修改 Prompt。请保留必要的 JSON 格式指令。
4. **分段大小**：默认为 5万字/段。对于超长篇小说，建议适当调大此数值以减少 API 调用次数。

### 第三步：开始分析
- **方式一：一键全书 (推荐)**
  点击左侧边栏的 **“一键全书分析”** 按钮。系统将自动从第一章开始逐章分析。
  *提示：您可以继续阅读，分析进度会以绿色圆点显示在章节列表中。*
- **方式二：手动分析**
  阅读时点击右侧“开始分析”。**注意**：必须先分析上一章，才能分析当前章，以确保剧情连贯。
